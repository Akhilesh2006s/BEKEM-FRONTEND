...
import type { DailyCapDto, IndentLineItemDto, ProcurementDecisionDto, ProjectDto } from '@afios/shared';
...
export function ProcurementDecisionDetailPage({ listPath }: ProcurementDecisionDetailPageProps) {
  ...
  const { data: projects }const { data: projects } = useQuery({
    ...
    enabled: role === UserRole.COORDINATOR && (!!decision?.canCoordinatorReview || !!decision?.canCoordinatorLocalClose),
  });
  ...
  const coordinatorLocalClose = useMutation({
    mutationFn: async (closeRemark: string) => {
    ...
    onSuccess: (data) => {
      if (data.escalated) {
        toast.error(data.message || 'Escalated to MD / Chairman — daily cap exceeded');
      } else {
  ...
  const canCoordinatorLocalClose =
    role === UserRole.COORDINATOR &&
    Boolean(
      decision?.canCoordinatorLocalClose ??
        (decision &&
          [
            'PENDING_HO',
  ...
  const coordinatorWithinDailyCap =
    canCoordinatorLocalClose &&
    (coordinatorCap ? (decision?.estimatedValue || 0) <= coordinatorCap.remaining : true);
  const requireRemark = () => {
    if (!remark.trim()) {
      ...
      >
        {decision && (
          <>
            ...
            {canCoordinatorLocalClose && (
              <div className="panel p-3 mb-3 space-y-3">
                <CoordinatorDailyCapBanner cap={coordinatorCap} />
                <p className="text-sm font-semibold text-ink">Coordinator local approval</p>
                <p className="text-xs text-ink-secondary">
                  {coordinatorWithinDailyCap
                ...
                <div>
                  <label className="text-xs font-medium text-ink-muted">Remark (required)</label>
                  <Textarea
                    value={remark}
                    onChange={(e) => setRemark(e.target.value)}
                ...
                <Button
                  ...
                  onClick={() => {
                    if (!requireRemark()) return;
                    coordinatorLocalClose.mutate(remark.trim());
                  }}
                >
                  {coordinatorWithinDailyCap
                    ? decision?.canFullyIssue
